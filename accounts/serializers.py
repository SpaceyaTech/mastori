...
class UserSerializer(serializers.ModelSerializer):
    ...
    # Added field to display the number of accounts that a user has.
    number_of_accounts = serializers.IntegerField(read_only=True)
    account = AccountDetailSerializer(many=True, read_only=True)

    class Meta:
        model = User
        fields = ['id', 'first_name', 'last_name', 'username', 'email',
                  'phone_number', 'password', 'number_of_accounts', 'account']
...
class UserAccountRegistrationSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if password == confirm_password:
            with transaction.atomic():
                user = User.objects.create_user(username=user['username'], first_name=user['first_name'],
                                                last_name=user['last_name'], email=user['email'], phone_number=user['phone_number'], password=password)

                account = Account.objects.create(
                    user=user, account_name=self.validated_data['account_name'], bio=self.validated_data['bio'])
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'confirm_password',
                  'account_name', 'display_picture', 'bio']
...
class AddAccountSerializer(serializers.ModelSerializer):
    def create(self, validated_data):
        ...
        account = Account.objects.create(
            user=user, account_name=account_name, bio=bio)
...
# Serializer to enable users login to their accounts
